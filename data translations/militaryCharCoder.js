function initMilitaryCharCoder(){
    ...
    function createBasicContainer(params){
        ...
        inlineStyler(cont,`{position: fixed; top: ${params?.top ? params?.top+'px' : '0px'}; width: ${params?.width ? params?.width+'px' : main_width}px; left: ${params?.left ? params?.left+'px' : '0px'}; z-index: ${topZIndexer()}; background: ${params?.bg_color ? params?.bg_color : 'transparent'}; border-radius: 0.2em; border: 2px solid ${params.border_color ? params.border_color : 'transparent'}; box-shadow:0px 1px 1px 1px rgba(0,0,0,0.4);}`);
    ...
    cont_elms.middle_cont.appendChild(go_btn);
    go_btn.onclick = ()=> {
        if(textarea.value?.trim()?.length){
            textarea.value = stringToMilitaryLetterCallout(textarea.value?.trim());
            textarea.select();
    ...
    function stringToMilitaryLetterCallout(s){
        var codes = [
            ...
            'Wiskey','Xray','Yankee',
            'Zulu'].map(r=> ({x:new RegExp(r[0],'i'),code:r}));
