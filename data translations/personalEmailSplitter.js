...
function personalEmailSort(){
    ...
    var split_emails = col.split(/\n/).map(r=> /\[/.test(r) ? JSON.parse(r.trim().replace(/'/g,'"')) : ['']).map(r=> r.filter(t=> p_o ? xx.test(t) : t).sort((a,b)=> ix(main_domains,a) - ix(main_domains,b))).map(r=> r.length ? r : ['']).map(r=> unqHsh(r.map(r=> r.toLowerCase()),{}).reduce((a,b)=> a+'\t'+b) ).reduce((a,b)=> a+'\n'+b);
