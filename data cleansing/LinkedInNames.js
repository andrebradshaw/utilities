...
var fixCase = (s)=> typeof s == 'string' && /^[A-Z]+$/.test(s.replace(/\W+/g, '').trim()) ? s.split(/\b-\b/).map(el=> el.replace(/\w\S*/g, (txt) => txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase())).join('-') : s;
